...
def find_urls(s):
    #Your code here
    urls = re.findall('http[s]?://+[^\s]+\.[a-z]{2,}', s)
    return urls
...
def get_umsi_data():
    ...
    for x in range(13):
    	base_url = "https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastna me_value=&rid=All&page={}".format(x)
    	html = requests.get(base_url, headers={'User-Agent': 'SI_CLASS'})
    	soup = BeautifulSoup(html.text, 'html.parser')

    	divs = soup.find_all('div', {'class':'view-content'})
    	people = divs[0].find_all('div', {'class':'field-item even','property':"dc:title"})
